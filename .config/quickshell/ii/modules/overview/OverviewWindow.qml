...
Item { // Window
    ...
    property real widthRatio: {
        ...
        // console.log(`w${widgetMonitor.id} '${widgetMonitor.transform}: ${widgetMonitor.width}x${widgetMonitor.height}`);
        // console.log(`m${monitorData.id} '${monitorData.transform}: ${monitorData.width}x${monitorData.height}`);
    ...
    property var targetWindowWidth: {
        let width = windowData?.size[0] * scale * widthRatio;
        // console.log(`width ${width}(${widthRatio}) on monitor ${widgetMonitor.id}`);
        width;
    }
    property var targetWindowHeight: {
        let height = windowData?.size[1] * scale * heightRatio;
        // console.log(`height ${height}(${heightRatio}) on monitor ${widgetMonitor.id}`);
        height;
    }
